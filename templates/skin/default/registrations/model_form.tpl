...
<form action="{router page='fashion_register/model'}" method="post" id="register-model-form">
  ...
  {assign var="profile_firstname_request" value=$_aRequest.profile_firstname}
  ...
  <input type="text" name="{$profile_firstname}" id="registration-{$profile_firstname}" value="{$profile_firstname_request}" class="input-text input-width-300 js-ajax-validate" />
  ...
  {assign var="profile_secondname_request" value=$_aRequest.profile_secondname}
  ...
  <input type="text" name="{$profile_secondname}" id="registration-{$profile_secondname}" value="{$profile_secondname_request}" class="input-text input-width-300 js-ajax-validate" />
  ...
  *}
  ...
  {assign var="profile_experience_request" value=$_aRequest.profile_experience}
  ...
  <select name="{$profile_experience}" id="{$profile_experience}" class="input-width-full js-ajax-validate">
    ...
    {foreach from=$profile_experience_list item=Key key=Name}
      <option value="{$Key}" {if $profile_experience_request==$Key}selected{/if}>{$Name|escape:'html'}</option>
    {/foreach}
  </select>
  ...
  {assign var="profile_gender_request" value=$_aRequest.profile_gender}
  ...
  <select name="{$profile_gender}" id="{$profile_gender}" class="input-width-full js-ajax-validate">
    ...
    {foreach from=$profile_gender_list item=Key key=Name}
      <option value="{$Key}" {if $profile_gender_request==$Key}selected{/if}>{$Name|escape:'html'}</option>
    {/foreach}
  </select>
